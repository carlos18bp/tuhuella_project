...
> **Tuhuella override**: 100% of DRF views in `backend/base_feature_app/views/` are **function-based with `@api_view`**. Pattern: queryset filter (by user permissions) → serializer validation → save → response. Never convert to CBV/APIView/ViewSets unless explicitly requested. The Django **module is `base_feature_project`** (a generic boilerplate name shared with `fernando_aragon_project`), and the app is `base_feature_app`.
...
2. **Run only modified test files**: `pytest base_feature_app/tests/path/to/test_file.py -v`. `pytest.ini` sets `DJANGO_SETTINGS_MODULE=base_feature_project.settings`.
3. **Maximum per execution**: 20 tests per batch, 3 commands per cycle.
4. The `base_feature_app` and the vendored `django_attachments` apps both have `tests/` directories.
