...
export default function AnimalDetailPage() {
    ...
    return (
      <div className="mx-auto max-w-[1400px] px-6 py-10">
        <div className="animate-pulse space-y-6">
          ...
          <div className="aspect-video bg-stone-100 rounded-2xl" />
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-6 py-10">
      <Link href={ROUTES.ANIMALS} className="text-sm text-teal-600 hover:text-teal-700">
        &larr; Volver a animales
      </Link>
            ...
            {isAuthenticated ? (
              <>
                <Link
                  ...
                  className="bg-teal-600 text-white rounded-full px-6 py-3 font-medium hover:bg-teal-700 transition-colors"
                >
                  Solicitar Adopción
                </Link>
                <Link
                  href={`${ROUTES.CHECKOUT_SPONSORSHIP}?animal=${animal.id}`}
                  className="border border-amber-300 text-amber-700 rounded-full px-6 py-3 font-medium hover:bg-amber-50 transition-colors"
                >
                  Apadrinar
                </Link>
              </>
