  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-8">
      ...
      <div className="flex gap-1 mb-6 bg-surface-tertiary rounded-xl p-1 max-w-xs">
        ...
        <button type="button" className={`flex-1 px-4 py-2 text-sm rounded-lg transition-all ${activeTab === 'json' ? 'bg-surface-primary shadow-sm font-medium text-text-primary' : 'text-text-tertiary'}`} onClick={() => setActiveTab('json')}>JSON Content</button>
      </div>
      ...
      <div className="space-y-6">
        ...
        {activeTab === 'json' && (
          <>
            <fieldset className="border border-border-primary rounded-xl p-5 space-y-4">
              <legend className="text-sm font-medium text-text-secondary px-2">Contenido JSON (ES)</legend>
              <textarea rows={15} value={jsonEsText} onChange={(e) => setJsonEsText(e.target.value)} className={`${inputClass} font-mono resize-y`} />
            </fieldset>
            <fieldset className="border border-border-primary rounded-xl p-5 space-y-4">
              <legend className="text-sm font-medium text-text-secondary px-2">Content JSON (EN)</legend>
        ...
        <fieldset className="border border-border-primary rounded-xl p-5 space-y-4">
          ...
          <div className="flex items-center gap-6">
            <label className="flex items-center gap-2 text-sm text-text-secondary"><input type="checkbox" checked={form.is_published || false} onChange={(e) => handleChange('is_published', e.target.checked)} className="rounded border-border-secondary text-teal-600 focus:ring-teal-500" />Publicado</label>
            <label className="flex items-center gap-2 text-sm text-text-secondary"><input type="checkbox" checked={form.is_featured || false} onChange={(e) => handleChange('is_featured', e.target.checked)} className="rounded border-border-secondary text-teal-600 focus:ring-teal-500" />Destacado</label>
          </div>
        </fieldset>
        ...
        <fieldset className="border border-border-primary rounded-xl p-5 space-y-4">
          <legend className="text-sm font-medium text-text-secondary px-2">Fuentes</legend>
          <p className="text-xs text-text-quaternary">Formato JSON: [{'"name"'}: {'"..."'}, {'"url"'}: {'"..."'}]</p>
          <textarea
            rows={4}
