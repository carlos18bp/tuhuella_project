  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-8">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className="text-2xl font-bold text-stone-800">Nuevo Blog Post</h1>
      </div>

      <div className="flex gap-1 mb-6 bg-stone-100 rounded-xl p-1 max-w-xs">
        <button type="button" className={`flex-1 px-4 py-2 text-sm rounded-lg transition-all ${mode === 'manual' ? 'bg-white shadow-sm font-medium text-stone-900' : 'text-stone-500 hover:text-stone-700'}`} onClick={() => setMode('manual')}>Manual</button>
        <button type="button" className={`flex-1 px-4 py-2 text-sm rounded-lg transition-all ${mode === 'json' ? 'bg-white shadow-sm font-medium text-stone-900' : 'text-stone-500 hover:text-stone-700'}`} onClick={() => setMode('json')}>Importar JSON</button>
      </div>
        ...
        <form onSubmit={handleManualSubmit} className="space-y-6">
          ...
          <fieldset className="border border-stone-200 rounded-xl p-5 space-y-4">
            ...
            <div className="flex items-center gap-6">
              <label className="flex items-center gap-2 text-sm text-stone-700"><input type="checkbox" checked={form.is_published} onChange={(e) => handleChange('is_published', e.target.checked)} className="rounded border-stone-300 text-teal-600 focus:ring-teal-500" />Publicado</label>
              <label className="flex items-center gap-2 text-sm text-stone-700"><input type="checkbox" checked={form.is_featured} onChange={(e) => handleChange('is_featured', e.target.checked)} className="rounded border-stone-300 text-teal-600 focus:ring-teal-500" />Destacado</label>
            </div>
          </fieldset>
      ...
      {mode === 'json' && (
        <form onSubmit={handleJsonSubmit} className="space-y-6">
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm text-stone-500">Pega el JSON completo del post o descarga el template.</p>
