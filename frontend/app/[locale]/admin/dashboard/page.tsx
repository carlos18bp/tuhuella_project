...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-6 py-10">
      ...
      {loading ? (
        <div data-testid="loading-skeleton" className="mt-8 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        <>
          <div className="mt-8 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {cards.map((card) => (
              <div key={card.label} className="rounded-xl border border-border-primary bg-surface-primary p-5 shadow-sm hover:shadow-md transition-shadow">
          ...
          <h2 className="mt-12 text-xl font-semibold text-text-primary">Acciones rápidas</h2>
        ...
        <p className="mt-8 text-text-quaternary">No se pudieron cargar las métricas.</p>
      )}
    </div>
  );
}
