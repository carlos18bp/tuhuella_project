    ...
    <div className="mx-auto max-w-[1400px] px-6 py-10 min-w-0 overflow-x-hidden">
      ...
      {loading ? (
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            { value: `$${Number(metrics.donations.total_amount).toLocaleString()}`, label: t('totalDonationsRaised'), detail: `${metrics.donations.total_count} ${t('paidDonations')}`, card: 'border-amber-200 bg-amber-50 dark:bg-surface-tertiary dark:border-amber-500/20', value_c: 'text-amber-700 dark:text-amber-400', label_c: 'text-amber-600 dark:text-amber-400/80', detail_c: 'text-amber-500 dark:text-amber-400/70' },
            { value: `$${Number(metrics.sponsorships.total_amount).toLocaleString()}`, label: t('totalSponsorships'), detail: `${metrics.sponsorships.total_count} ${t('activeSponsorships')}`, card: 'border-teal-200 bg-teal-50 dark:bg-surface-tertiary dark:border-teal-500/20', value_c: 'text-teal-700 dark:text-teal-400', label_c: 'text-teal-600 dark:text-teal-400/80', detail_c: 'text-teal-500 dark:text-teal-400/70' },
            { value: `${adoptionRate}%`, label: t('adoptionRate'), detail: `${metrics.adoption_rate.total_adopted} / ${metrics.adoption_rate.total_published}`, card: 'border-emerald-200 bg-emerald-50 dark:bg-surface-tertiary dark:border-emerald-500/20', value_c: 'text-emerald-700 dark:text-emerald-400', label_c: 'text-emerald-600 dark:text-emerald-400/80', detail_c: 'text-emerald-500 dark:text-emerald-400/70' },
