  ...
  return (
    <div className="mx-auto max-w-[1400px] px-6 py-10">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-stone-800">Gestión de Animales</h1>
          <p className="mt-1 text-stone-500">{animals.length} animales registrados</p>
        </div>
        ...
        {['all', 'draft', 'published', 'in_process', 'adopted', 'archived'].map((s) => (
          <button key={s} onClick={() => setFilter(s)}
            className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${
              filter === s ? 'bg-teal-600 text-white border-teal-600' : 'border-stone-200 text-stone-600 hover:bg-stone-50'
            }`}>
      ...
      {loading ? (
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="rounded-2xl border border-stone-200 p-5 animate-pulse space-y-3">
        ...
        <p className="mt-8 text-stone-400">No hay animales con este filtro.</p>
      ) : (
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.map((animal: Animal) => {
            const st = statusLabels[animal.status] ?? { label: animal.status, color: 'bg-stone-100 text-stone-600' };
            return (
