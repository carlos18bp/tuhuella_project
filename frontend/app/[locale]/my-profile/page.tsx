...
type PendingIcon = ComponentType<{ className?: string }>;

function PendingWidget({ icon: Icon, href, label }: { icon: PendingIcon; href: string; label: string }) {
  const t = useTranslations('profile');
  return (
    <Link
      href={href}
      className="group flex items-start gap-3 rounded-xl border border-amber-200 dark:border-amber-800/60 bg-gradient-to-br from-amber-50 to-surface-secondary dark:from-amber-900/15 dark:to-surface-secondary p-4 hover:shadow-sm transition-all"
    >
      <div className="h-10 w-10 rounded-lg bg-amber-100/80 dark:bg-amber-900/30 flex items-center justify-center shrink-0">
        <Icon className="h-5 w-5 text-amber-600 dark:text-amber-400" />
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-text-primary">{label}</p>
        <p className="mt-1 inline-flex items-center text-xs font-medium text-teal-600 dark:text-teal-400 group-hover:underline">
          {t('viewAll')}
...
export default function MiPerfilPage() {
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-6 py-10 min-w-0 overflow-x-hidden">
      {isAdopter && stats && stats.shelter_invites.pending_count > 0 && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        <div className="lg:col-span-1 space-y-6">
          ...
          {user.role === 'shelter_admin' && (
            <PendingWidget
              ...
              label={
                roleData.shelterStats?.pending_applications
                  ? t('shelterPendingWidget', { count: roleData.shelterStats.pending_applications })
                  : t('shelterPendingWidgetZero')
              }
            />
          )}
          {user.role === 'admin' && (
            <PendingWidget
              ...
              label={
                roleData.adminStats?.pending_verifications
                  ? t('adminQueueWidget', { count: roleData.adminStats.pending_verifications })
                  : t('adminQueueWidgetZero')
              }
            />
          )}
        </div>
        ...
        <div className="lg:col-span-2">
          <h2 className="text-xl font-bold text-text-primary mb-4">{rightColumnHeading}</h2>

          {isAdopter && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {adopterActivityLinks.map((item) => {
          ...
          {user.role === 'shelter_admin' && (
            <ShelterAdminProfileSection
              shelter={roleData.shelter}
              stats={roleData.shelterStats ?? undefined}
            />
          )}
