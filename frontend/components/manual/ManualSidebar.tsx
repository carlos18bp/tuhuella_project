        ...
        return (
          <div key={section.id} className="flex flex-col">
            <button
              ...
              className={`group flex items-center justify-between gap-2 rounded-lg px-3 py-2 text-left text-sm font-semibold transition-colors ${
                section.highlight
            ...
            {!isCollapsed && (
              <ul id={`manual-section-${section.id}`} className="mt-1 flex flex-col gap-0.5 pl-6">
                {section.processes.map((process) => (
                  <li key={process.id}>
                    <a
                      href={`#${process.id}`}
                      onClick={() => setMobileOpen(false)}
                      className="block rounded-md border-l-2 border-transparent px-2 py-1.5 text-sm text-text-secondary transition-colors hover:bg-surface-hover hover:text-text-primary"
                    >
  ...
  return (
    <>
      ...
      <aside className="hidden lg:block w-72 flex-shrink-0">
        <div className="sticky top-24 max-h-[calc(100vh-7rem)] overflow-y-auto rounded-2xl border border-border-primary bg-surface-primary p-4">
          <h2 className="mb-3 px-2 text-xs font-semibold uppercase tracking-wider text-text-quaternary">
            {label}
          </h2>
          {nav}
        </div>
