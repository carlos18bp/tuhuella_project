...
export default function ManualSearch({ locale }: Props) {
  ...
  return (
    <div className="relative">
      <label className="flex items-center gap-2 rounded-xl border border-border-primary bg-surface-primary px-4 py-2.5 focus-within:border-teal-500 focus-within:ring-2 focus-within:ring-teal-500/20">
        <Search className="h-4 w-4 text-text-quaternary" aria-hidden="true" />
        <input
          ref={inputRef}
          type="search"
          role="searchbox"
        ...
        <kbd className="hidden sm:inline-flex items-center gap-0.5 rounded border border-border-secondary bg-surface-secondary px-1.5 py-0.5 text-[10px] font-medium text-text-quaternary">
          ⌘K
        </kbd>
      </label>
      ...
      {isSearching && (
        <div
          role="listbox"
          className="absolute left-0 right-0 top-full z-40 mt-2 max-h-96 overflow-y-auto rounded-2xl border border-border-primary bg-surface-elevated p-2 shadow-xl dark:bg-surface-elevated/95 dark:backdrop-blur-xl"
        >
          {results.length === 0 ? (
            <p className="px-4 py-6 text-center text-sm text-text-quaternary">{k('noResults')}</p>
          ) : (
                ...
                <li key={hit.process.id}>
                  ...
                  >
                    <span className="flex items-center justify-between gap-2">
                      ...
                      <RoleBadge audience={hit.process.audience} />
                    </span>
                    <span className="text-xs text-text-secondary line-clamp-2">
                      {hit.process.summary[locale]}
                    </span>
                    {hit.process.route && (
                      <code className="inline-block w-fit rounded-md bg-surface-secondary px-1.5 py-0.5 text-[11px] text-text-quaternary">
                        {hit.process.route}
                      </code>
                    )}
                  </button>
