...
export default function AnimalCard({ animal, compact = false }: AnimalCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className={`${compact ? 'p-3' : 'p-5'} flex-1 flex flex-col`}>
        <h3 className={`font-semibold text-text-primary group-hover:text-teal-600 transition-colors ${compact ? 'text-sm' : ''}`}>
          {animal.name}
        </h3>
