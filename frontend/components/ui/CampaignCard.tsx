...
export default function CampaignCard({ campaign }: CampaignCardProps) {
  return (
    ...
    >
      <div className="aspect-[16/9] bg-gradient-to-br from-amber-50 to-stone-100 dark:from-amber-950/35 dark:to-surface-tertiary relative overflow-hidden">
        {campaign.cover_image_url ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center text-stone-300 dark:text-amber-600/45">
      ...
      <div className="p-6 flex-1 flex flex-col">
        <div className="flex items-center gap-2">
          ...
          <p className="text-xs text-amber-600 dark:text-amber-400 font-medium">{campaign.shelter_name}</p>
        </div>
        <h3 className="mt-2.5 font-semibold text-text-primary group-hover:text-amber-700 dark:group-hover:text-amber-300 transition-colors">{campaign.title}</h3>
        {campaign.description && (
          <p className="mt-2 text-sm text-text-tertiary line-clamp-2">{campaign.description}</p>
        )}
        <div className="mt-auto pt-5">
          <div className="flex justify-between text-xs mb-1.5">
            <span className="font-semibold text-amber-700 dark:text-amber-400">{campaign.progress_percentage}%</span>
